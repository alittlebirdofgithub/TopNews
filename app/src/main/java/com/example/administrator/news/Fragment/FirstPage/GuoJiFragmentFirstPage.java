...
public class GuoJiFragmentFirstPage extends FirstPageSubBaseFragment {
    public static final String GUONJI_BEAN_FILE= FileCache.Save_Directory+"/Bean/guoji.txt";
    public GuoJiFragmentFirstPage(Context context) {
        super(context);
    }
    ...
    public void handleMessage(Message msg) {
        ...
        BaseFragment.defaultCacheUtil.dealAddCache(DefaultCacheUtil.BEAN_FILE,GUONJI_BEAN_FILE,newsBeen);
        //如果不相等，说明子Fragment切换过快（当数据发送到handler时,子Fragment改变了），不再是原来请求数据的那个子Fragment。所以不响应更新UI。
        if(msg.what== SubFragmentFactory.GuoJi)
    ...
    public void handleInternetErrorMessage(Message msg) {
        ...
        if(type==SubFragmentFactory.GuoJi)
        {
            ArrayList<NewsBean> arrayList = (ArrayList<NewsBean>) BaseFragment.defaultCacheUtil.dealGetCache(DefaultCacheUtil.BEAN_FILE,GUONJI_BEAN_FILE);
            if(arrayList!=null) {
