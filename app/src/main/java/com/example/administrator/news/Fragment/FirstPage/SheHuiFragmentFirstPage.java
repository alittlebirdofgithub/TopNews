...
public class SheHuiFragmentFirstPage extends FirstPageSubBaseFragment {
    public static final String SHEHUI_SAVE_FILE= FileCache.Save_Directory+"/Bean/shehui.txt";
    public SheHuiFragmentFirstPage(Context context) {
        super(context);
    }
    /**
     * 处理请求成功的社会信息，
    ...
    public void handleMessage(Message msg) {
        ...
        BaseFragment.defaultCacheUtil.dealAddCache(DefaultCacheUtil.BEAN_FILE,SHEHUI_SAVE_FILE,newsBeen);
        //如果不相等，说明子Fragment切换过快（当数据发送到handler时,子Fragment改变了），不再是原来请求数据的那个子Fragment。所以不响应更新UI。
        if(msg.what== SubFragmentFactory.SheHui)
    ...
    public void handleInternetErrorMessage(Message msg) {
        ...
        if(type==SubFragmentFactory.SheHui)
        {
            ArrayList<NewsBean> arrayList = (ArrayList<NewsBean>) BaseFragment.defaultCacheUtil.dealGetCache(DefaultCacheUtil.BEAN_FILE,SHEHUI_SAVE_FILE);
            if(arrayList!=null) {
